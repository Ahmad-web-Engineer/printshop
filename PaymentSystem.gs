...
function adminApprovePayment(payId, orderId, token) {
  ...
  try {
    ...
    _safeSetOrderStatus(orderId, 'Accepted', 'Paid');
    const ledger = getData('PaymentLedger');
    ...
    try {
      ...
      if (o && o.user_email) {
        ...
        if (u) _sendPaymentStatusEmail(u, orderId, 'Approved', '');
      }
    } catch(e) {}
    return { ok:true };
  } catch(e) { return { ok:false, err:e.message }; }
...
function adminRejectSmartPayment(payId, orderId, reason, token) {
  ...
  try {
    ...
    setCell('PaymentLedger', ri, 15, now);
    setCell('PaymentLedger', ri, 16, 'Admin');
    _safeSetOrderStatus(orderId, 'Payment Rejected', 'Rejected');
    const ledger = getData('PaymentLedger');
    const pay    = ledger.find(p => String(p.payment_id) === String(payId));
    if (pay) _logFraud(orderId, String(pay.user_id)||'', String(pay.upi_ref)||'', 'ADMIN_REJECTED: ' + (reason||''));
    try {
      const o = getData('Orders').find(x => String(x.order_id) === String(orderId));
      if (o && o.user_email) {
        const u = getData('Users').find(u => String(u.email).toLowerCase() === String(o.user_email).toLowerCase());
        if (u) _sendPaymentStatusEmail(u, orderId, 'Rejected', reason||'');
      }
    } catch(e) {}
    return { ok:true };
  } catch(e) { return { ok:false, err:e.message }; }
...
function getPaymentStatus(orderId, userId, token) {
  ...
  try {
    ...
    const order = getData('Orders').find(o => String(o.order_id) === String(orderId));
...
function checkSessionExpiry(sessionId, orderId, userId, token) {
  if (!checkUserSession(userId, token)) return { ok:false, err:'Session expired' };
  try {
    const session = getData('PaymentSessions').find(s => String(s.session_id) === String(sessionId));
